...
public class Sc61860_1360 extends  Sc61860Base {
    ...
    protected void LoadRomImage(Context c) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            File f = new File(old_rom_path+"/pc1360mem.bin");
            uri = Uri.fromFile(f);
        } else {
            DocumentFile src = rom_dir.findFile("pc1360mem.bin");
            if (src != null) {
                uri = src.getUri();
            }
        }
        ...
        uri = null;
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            File f = new File(old_rom_path+"/pc1360bank.bin");
            uri = Uri.fromFile(f);
        } else {
            DocumentFile src = rom_dir.findFile("pc1360bank.bin");
            if (src != null) {
                uri = src.getUri();
            }
        }
