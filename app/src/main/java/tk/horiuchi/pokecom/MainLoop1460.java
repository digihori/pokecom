...
public class MainLoop1460 extends MainLoopBase {
    private final int digit=5*24;
    ...
    public MainLoop1460(Context context, SurfaceView sv) {
        super(context, sv);
    ...
    public void surfaceChanged(SurfaceHolder holder, int f, int w, int h) {
        Log.w("!!!1460!!!", String.format("width = %d, height=%d\n", w, h));
        float s1 = (float)h / 66f;
        float s2 = (float)w * 0.94f / 429f;
        dpdx = s1 < s2 ? s1 : s2;
        Log.w("!!!1460!!!", String.format("dpdx = %f (%f, %f)\n", dpdx, s1, s2));
    ...
    protected void doDraw(SurfaceHolder holder) {
        ...
        if (c != null) {
            ...
            if ((state[0] & dispMATRIX) != 0) {
                p.setColor(Color.DKGRAY);
            } else {
                p.setColor(Color.LTGRAY);
            }
            c.drawText("MATRIX", 300, 64, p);
            if ((state[0] & dispSTAT) != 0) {
                p.setColor(Color.DKGRAY);
            } else {
                p.setColor(Color.LTGRAY);
            }
            c.drawText("STAT", 352, 64, p);
            if ((state[2] & dispPRINT) != 0) {
                p.setColor(Color.DKGRAY);
            } else {
                p.setColor(Color.LTGRAY);
            }
            c.drawText("PRINT", 388, 64, p);
