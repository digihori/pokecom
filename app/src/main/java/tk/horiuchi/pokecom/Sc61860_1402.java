...
public class Sc61860_1402 extends Sc61860Base {

    public Sc61860_1402(Context c) {
        super(c);
    ...
    protected void LoadRomImage(Context c) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            File f = new File(old_rom_path+"/pc1402mem.bin");
            uri = Uri.fromFile(f);
        } else {
            DocumentFile src = rom_dir.findFile("pc1402mem.bin");
            if (src != null) {
                uri =src.getUri();
            }
        }
