...
public class MainLoop1350 extends MainLoopBase {
    ...
    public MainLoop1350(Context context, SurfaceView sv) {
        super(context, sv);
    ...
    public void surfaceChanged(SurfaceHolder holder, int f, int w, int h) {
        Log.w("!!!1350!!!", String.format("width = %d, height=%d\n", w, h));
        float s1 = (float)h * 0.80f / 93f;
        float s2 = (float)w * 0.86f / 453f;
        dpdx = s1 < s2 ? s1 : s2;
        Log.w("!!!1350!!!", String.format("dpdx = %f (%f, %f)\n", dpdx, s1, s2));
